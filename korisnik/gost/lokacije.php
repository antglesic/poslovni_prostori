...
$sql_upit_lokacije = "SELECT * FROM lokacija";
$lokacije = $baza->selectDB($sql_upit_lokacije);

$head = "<thead>" . "<tr>" . "<th>Ime lokacije</th>" . "<th>Adresa</th>" . "<th>Odabir</th>" . "</tr>" . "</thead>";
$table = "";

while ($row = $lokacije->fetch_assoc()) {
    $table = $table . "<tr>";
    $table = $table . "<td>" . $row["ime_lokacije"] . "</td>" . "<td>" . $row["adresa"] . "</td>" . "<td>" . "<a href=dvorane.php?idLokacije=" . $row["idlokacija"] . ">Pregled</a>" . "</td>";
    $table = $table . "</tr>";
}
