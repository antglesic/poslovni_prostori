...
$sql_upit_korisnici = "SELECT * FROM korisnik WHERE korisnicko_ime NOT LIKE '" . $_SESSION["korisnik"] . "'";
$korisnici = $baza->selectDB($sql_upit_korisnici);

$head = "<thead>" . "<tr>" . "<th>Ime i prezime</th>" . "<th>Status računa</th>" . "<th>Otključaj</th>" . "<th>Zaključaj</th>" . "</tr>" . "</thead>";
$table = "";
...
while ($row = mysqli_fetch_array($korisnici)) {
    $table = $table . "<tr>";
    if($row["status"] == '0') {
        $table = $table . "<td>" . $row["ime"] . " " . $row["prezime"] . "</td>" . "<td>Neaktivan</td>" . "<td><a href='obradaKorisnika.php?idKorisnika=" . $row["idkorisnik"] . "&operacija=1'>OTKLJUČAJ</a></td>" . "<td><a href='obradaKorisnika.php?idKorisnika=" . $row["idkorisnik"] . "&operacija=2'>ZAKLJUČAJ</a></td>";
    }
    if($row["status"] == '1') {
        $table = $table . "<td>" . $row["ime"] . " " . $row["prezime"] . "</td>" . "<td>Aktivan</td>" . "<td><a href='obradaKorisnika.php?idKorisnika=" . $row["idkorisnik"] . "&operacija=1'>OTKLJUČAJ</a></td>" . "<td><a href='obradaKorisnika.php?idKorisnika=" . $row["idkorisnik"] . "&operacija=2'>ZAKLJUČAJ</a></td>";
    }
    if($row["status"] == '2') {
        $table = $table . "<td>" . $row["ime"] . " " . $row["prezime"] . "</td>" . "<td>Zaključan</td>" . "<td><a href='obradaKorisnika.php?idKorisnika=" . $row["idkorisnik"] . "&operacija=1'>OTKLJUČAJ</a></td>" . "<td><a href='obradaKorisnika.php?idKorisnika=" . $row["idkorisnik"] . "&operacija=2'>ZAKLJUČAJ</a></td>";
    }
    $table = $table . "</tr>";
}
...
<html lang="hr">
    <head>
        ...
        <title>Upravljanje korisničkim računima</title>
    </head>
    <body>
        <div id="cssmenu">
            <nav>
